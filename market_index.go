...
type Market struct {
        ...
        // CurrentPrice is the live YES probability (0.0–1.0) fetched from the real API.
...
// attachPrice fills in CurrentPrice from the prices map if not already set.
func (mi *MarketIndex) attachPrice(m Market) Market {
        if m.CurrentPrice == 0 {
                key := strings.ToLower(m.Question)
                if p, ok := mi.prices[key]; ok {
                        m.CurrentPrice = p
                } else if p, ok := mi.prices[m.ID]; ok {
                        m.CurrentPrice = p
                }
        }
        return m
...
func (mi *MarketIndex) Search(query string, f MarketFilters) []Market {
        ...
        for _, m := range mi.markets {
                ...
                results = append(results, mi.attachPrice(m))
        }
