...
// LivePosition wraps a Position snapshot with real-time P&L fields.
type LivePosition struct {
        Position
        PnL        float64 `json:"pnl"`
        PnLPercent float64 `json:"pnlPercent"`
        PnLDelta   float64 `json:"pnlDelta"` // change since last poll (always 0 in server-side snapshots, used by frontend)
}
...
type PositionManager struct {
        ...
        simulatePx    bool // enable random-walk price simulation (demo mode)
}
...
func NewPositionManager(checkIntervalMs int) *PositionManager {
        ...
        return &PositionManager{
                ...
                simulatePx:    true, // enable demo price simulation (disable when real feeds are connected)
        }
}
...
// ListPositionsLive returns all positions enriched with current P&L.
func (pm *PositionManager) ListPositionsLive(platform string) []LivePosition {
        pm.mu.RLock()
        defer pm.mu.RUnlock()
        ...
        for _, p := range pm.positions {
                ...
                pnl := pm.calculatePnL(p)
                result = append(result, LivePosition{
                        Position:   *p,
                        PnL:        math.Round(pnl.PnL*100) / 100,
...
// UpdatePriceManual allows callers to manually set a current price (e.g. from a real feed).
func (pm *PositionManager) UpdatePriceManual(positionID string, price float64) error {
        pm.mu.Lock()
        defer pm.mu.Unlock()
        pos, ok := pm.positions[positionID]
        if !ok {
...
// simulatePrices applies a small random walk to every position's current price.
// Prediction market prices are probabilities, so they're clamped to [0.01, 0.99].
// Volatility: ±0.5% per 5s tick (~±7% per hour drift rate in demo mode).
func (pm *PositionManager) simulatePrices() {
        pm.mu.Lock()
        defer pm.mu.Unlock()
        for _, pos := range pm.positions {
                // Gaussian-like change: sum of 2 uniform samples gives bell-shaped drift
                drift := (rand.Float64() - 0.5 + rand.Float64() - 0.5) * 0.005
                newPrice := pos.CurrentPrice * (1 + drift)
                // Clamp to valid prediction market probability range
                if newPrice < 0.01 {
                        newPrice = 0.01
                } else if newPrice > 0.99 {
                        newPrice = 0.99
                }
                newPrice = math.Round(newPrice*10000) / 10000 // 4 decimal places
                pos.CurrentPrice = newPrice
                pos.UpdatedAt = time.Now()
                if newPrice > pos.HighWaterMark {
                        pos.HighWaterMark = newPrice
                }
        }
}

func (pm *PositionManager) monitorPositions() {
        // Apply price simulation first so stop/TP checks use updated prices
        if pm.simulatePx {
                pm.simulatePrices()
        }
